...
def get(core_api, service_name):
    try:
        return core_api.read_namespaced_service(
            service_name, namespace="default")
...
def wait_for(core_api, service_name, seconds):
    service = None
    t_end = time.time() + seconds
    while time.time() < t_end:
        service = get(core_api, service_name)
        if service:
            break
        time.sleep(1)
    return service
...
def delete(core_api, service_name):
    core_api.delete_namespaced_service(
        service_name, "default")


def delete_if_exists(core_api, service_name):
    if get(core_api, service_name):
        logging.info("deleting service '%s' ..." % service_name)
        delete(core_api, service_name)


def create_load_balancers(core_api, stack_id, role):
    web = False
    mgmt = False
    splunktcp = False
    selector = {
    ...
    if role == indexer_role:
        ...
        for pod in pods:
            ...
            if not get(core_api, load_balancer_name):
        ...
        for load_balancer_name in lbs_to_delete:
            delete(core_api, load_balancer_name)
    else:
...
def delete_all_load_balancers(core_api, stack_id):
